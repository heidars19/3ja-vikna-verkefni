...
\n
    class StaffFile(data_to_append: [string or list]..., fieldname: [string]..., searchparam: [string or integer], line_to_replace: [string]..., replace_with: [string]... )    \n
\n
    Usage:\n
    Call class constructor with variable name. \n
    Append data \n
    MakeNewInstance = StaffFile(data_to_append=...) \n
    \n 
    Read data\n
    MakeNewInstance = StaffFile() \n
    --Empty constructor reads and returns a list\n
    
    Replace data\n
    StaffFile(line_to_replace=..., replace_with=...)\n
    \n
    Find data, returns line number, 0 if not found and -1 if error\n
    MakeNewInstance = StaffFile(fieldname=..., searchparam=...)
    '''

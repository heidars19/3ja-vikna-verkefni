...
def main ():
    ...
    # new_instance = DestinationFile(line_to_replace='Sessý,TEST,TEST,BAE,BAE', replace_with="TF-EOC,NAFokker80,Fokker,F800,Heiðar,100")
    # new_instance.start()    
    
    # # Change a line at a certain line number - check
    # new_instance = DestinationFile(line_to_replace=3, replace_with="Þetta ætti að breyta línu 3")
    # new_instance.start()

    # # Changes part of a line - check
    # new_instance = DestinationFile(line_to_replace="TF-OAS,NABAE146,BAE,146,Sessý,82", replace_with="TF-OAS,NABAE146,BAE,146,Sessý,182")
    # new_instance.start()
